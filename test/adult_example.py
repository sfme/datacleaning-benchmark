...
cat_mdl_overt = CategoricalNoiseModel((df_data.shape[0], len(cat_feat_names)), cats_names, 
                                cats_probs_list=[], typo_prob=0.15) 
...
# Define Mixed Model (0.10*0.30 = 0.03 of cells noised, and 0.10 of tuples)
mix_mdl_overt = MixedNoiseTupleWiseModel(df_data.shape, cat_array_bool, 
                                         idx_map_cat, idx_map_num, cat_mdl_overt,
                                         num_mdl_overt, probability=0.10, p_row=0.30)
...
noised_overt_outliers_df.to_csv(folder_path + name_file + "_noised_overt.csv", index=False)
...
cat_mdl_covert = CategoricalNoiseModel((df_data.shape[0], len(cat_feat_names)), cats_names, 
                                       cats_probs_list=cats_probs, typo_prob=0.05) 
...
mix_mdl_covert = MixedNoiseTupleWiseModel(df_data.shape, cat_array_bool, 
                                          ...
                                          num_mdl_covert, probability=0.10, p_row=0.30)
...
noised_covert_outliers_df.to_csv(folder_path + name_file + "_noised_covert.csv", index=False)
